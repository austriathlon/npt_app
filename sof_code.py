...
data = data[['program_id', 'event_id', 'program_date', 'program_name', 'race_level', 
...
data_male = data[data['athlete_gender'] == 'male']
...
data_female = data[data['athlete_gender'] == 'female']
data['program_date'] = pd.to_datetime(data['program_date'])
...
with tab1:
    st.subheader("Pick a name")

    # Select box for choosing a name
    if filtered_names:  # Only show the selectbox if there are names available
        selected_name = st.selectbox('test',filtered_names, index=default_index, label_visibility='hidden')
        st.markdown("</br>", unsafe_allow_html=True)
